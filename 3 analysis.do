...
forval j = 1/2 {
	...
	* append results
	...
	save "$temp\DK_nlift_model`modnum'.dta", replace 
}
...
forval j = 1/2 {
	...
	* append results
	...
	g model=`modnum'
	keep if regexm(var,"treat") | regexm(var,"cons") 
	save "$temp\CB05_nlift_model`modnum'.dta", replace 
}

* ==== CB07: OPINIONS ABOUT ECONOMY ==== *
** model 1
* open data
setupdataCB07
est clear
...
foreach outfam in `outfamlist' {
	forval model = 1/3 {
		if "`outfam'"=="CB05" & `model'==1 {
			...
			append using "$temp\\`outfam'_nlift_model`model'.dta"
		}
	}
}
g outfam=""
...
reshape wide coef, i(outcome model) j(var) string

* calculate probability of treatment
foreach x of varlist coef* {
